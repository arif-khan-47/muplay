...
import { addWatchTime, countView, addToHistoryEndpoint, updateHistoryEndpoint, getContentSignCookieEndPoint } from "../../http";
...
interface IVideoPlayerProps {
  ...
  type?: string;
  contentData: IAllContentResponse['data'][0];
  userSession?: ISessionData;
  isTrailer?: boolean;
  episode?: IEpisodeContentResponse | null;
}
...
const VideoPlayer = ({ sourceUrl, type, contentData, userSession, isTrailer, episode }: IVideoPlayerProps): JSX.Element => {
  ...
  const options = {
    ...
    sources: [
      {
        src: episode ? episode.source_link : isTrailer ? contentData.trailer_source_link : sourceUrl,
      },
    ],
  };
